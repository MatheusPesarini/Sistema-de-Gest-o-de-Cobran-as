            ...
            <div class="col-md-8">
                <div class="card shadow-sm">
                    <div class="card-header">
                        <h4 class="mb-0"><i class="bi-pencil-square"></i> Editar Cobrança #${cobranca?.id}</h4>
                    </div>
                    <div class="card-body">
                        ...
                        <g:form resource="${cobranca}" controller="cobranca" method="PUT">
                            <g:hiddenField name="version" value="${cobranca?.version}"/>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Cliente</label>
                                <g:select name="cliente.id"
                                          from="${clientes ?: [cobranca?.cliente]}"
                                          optionKey="id"
                                          optionValue="nome"
                                          value="${cobranca?.cliente?.id}"
                                          class="form-select"/>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold" for="descricao">Descrição</label>
                                <g:textField name="descricao" value="${cobranca?.descricao}" class="form-control"/>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold" for="valor">Valor (R$) <span class="text-danger">*</span></label>
                                <g:field type="number" name="valor" value="${cobranca?.valor}" class="form-control" step="0.01" min="0.01" required="required"/>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <g:select name="status"
                                          from="${['PENDENTE', 'PAGA', 'CANCELADA']}"
                                          value="${cobranca?.status}"
                                          class="form-select"/>
                            </div>
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi-floppy"></i> Atualizar
                                </button>
                                <g:link action="show" id="${cobranca?.id}" class="btn btn-outline-secondary">
